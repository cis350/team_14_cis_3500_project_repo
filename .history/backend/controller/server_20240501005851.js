...
webapp.post('/login', (req, resp) => {
  ...
  try {
    // This part simulates successful login by directly using the username from the request
    const token = jwt.sign({ username: req.body.username }, process.env.KEY, { expiresIn: '1h' });
    resp.status(201).json({ apptoken: token });
  } catch (err) {
    // In case something goes wrong with JWT token creation
    resp.status(400).json({ error: 'Failed to create token' });
  }
});
